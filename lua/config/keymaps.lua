...
local function terminal_open(win)
  require("snacks").terminal.open(nil, { win = win })
end

local function terminal_toggle(win)
  require("snacks").terminal.toggle(nil, { win = win })
end
...
map("n", "<leader>h", function()
  terminal_open({ position = "bottom", height = 0.3 })
end, { desc = "terminal new horizontal term" })
map("n", "<leader>v", function()
  terminal_open({ position = "right", width = 0.2 })
end, { desc = "terminal new vertical term" })
map({ "n", "t" }, "<M-v>", function()
  terminal_toggle({ position = "right", width = 0.2 })
end, { desc = "terminal toggleable vertical term" })
map({ "n", "t" }, "<M-h>", function()
  terminal_toggle({ position = "bottom", height = 0.3 })
end, { desc = "terminal toggleable horizontal term" })
map({ "n", "t" }, "<M-i>", function()
  terminal_toggle({ position = "float", width = 0.8, height = 0.8 })
