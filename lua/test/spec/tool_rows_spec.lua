describe("tool.rows", function()
  ...
  it("builds flat tool rows for lsp categories", function()
    local columns, row_items = rows.build(ui(), "lsp")

    assert.equals("Tool", columns[3].label)
    assert.is_true(#row_items > 0)
    for _, row in ipairs(row_items) do
      assert.equals("tool", row.entry.kind)
  ...
  it("builds grouped rows for ordered categories", function()
    ...
    for _, row in ipairs(row_items) do
      ...
      elseif row.entry.kind == "tool" then
        has_tool = true
        assert.equals("python", row.entry.ft)
  ...
  it("adds detail rows for expanded flat tools", function()
