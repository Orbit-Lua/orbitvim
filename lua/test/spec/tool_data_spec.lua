describe("tool.data", function()
  ...
  describe("content_lines", function()
    it("returns lsp count equal to vim.tbl_count(tools.lsp)", function()
      assert.equals(vim.tbl_count(tools.lsp), data.content_lines("lsp"))
    end)

    it("returns dap count equal to vim.tbl_count(tools.dap)", function()
  ...
  describe("build_ft_groups for formatter", function()
    ...
    it(
      "every formatter tool appears in its declared filetype groups",
      function()
        local groups = data.build_ft_groups("formatter")
        local by_ft = {}
  ...
  describe("tool_entries", function()
    it("returns sorted tools for a category", function()
    ...
    it("filters tools by filetype", function()
  ...
  describe("state_summary", function()
    it("counts total tools for a category", function()
  ...
  describe("build_ft_groups for linter", function()
    ...
    it("every linter tool appears in its declared filetype groups", function()
      ...
      for name, meta in pairs(tools.linter) do
        for _, ft in ipairs(meta.ft or {}) do
          assert.is_not_nil(by_ft[ft], "No linter group for ft '" .. ft .. "'")
          assert.is_true(
            vim.tbl_contains(by_ft[ft], name),
  ...
  describe("entry_status", function()
    it(
      ...
      function()
        ...
        local status, hl = data.entry_status("lsp", name, tools.lsp[name])
    ...
    it(
      ...
      function()
        ...
        local status, hl =
          data.entry_status("formatter", name, tools.formatter[name])
        assert.is_true(type(status) == "string" and #status > 0)
        assert.is_true(type(hl) == "string" and #hl > 0)
      end
    )
    ...
    it("returns n/a when mason registry is not available", function()
      ...
      local name, meta = next(tools.lsp)
