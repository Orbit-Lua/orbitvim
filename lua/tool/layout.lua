...
---@param name string
---@return boolean
function M.is_tool_expanded(ui, category, name)
...
function M.build_tabline(ui, win_width)
  ...
  local tab_ranges = {}
  for i, category_name in ipairs(cfg.tool_categories) do
    local lbl =
...
function M.scope_line(ui, category)
  ...
  if summary.installed then
    return string.format(
      "%s%s: %s%s%d installed%s%d missing%s%d %s",
      prefix,
      labels.tool_states,
