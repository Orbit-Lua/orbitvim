...
local function package_label(meta)
  return meta.mason or meta.source or cfg.labels.external
end
...
local function tool_icon(is_enabled, status_hl)
  if not is_enabled then
    return cfg.icons.disabled, "ToolMuted"
  end
...
function M.build_columns(category, icon_disp_w)
  ...
  local name_w = (category == "lsp" or category == "dap") and cfg.col_name
    or cfg.col_tool
  local name_label = core.is_ordered_category(category) and labels.grouped_tool
    or labels.tool
  return {
  ...
  for _, group in ipairs(data.build_ft_groups(category, layout.active_ft(ui))) do
    ...
    if is_expanded then
      for idx, name in ipairs(group.names) do
        local meta = tools[category][name]
        if meta then
          ...
          local icon, icon_hl = tool_icon(is_enabled, status_hl)
          table.insert(rows, {
            cells = {
              tree = "",
              icon = icon,
...
---@param category ToolCategory
---@return Tool.Table.Row[]
local function build_tool_rows(ui, category)
  local labels = cfg.labels
  local rows = {}
  for _, tool_entry in ipairs(data.tool_entries(category, layout.active_ft(ui))) do
    local name = tool_entry.name
    local meta = tool_entry.meta
    local handler = require("tool.category")[category]
    local is_enabled = not handler.capabilities.toggle
      or state_mod.is_enabled(category, name)
    local is_expanded = layout.is_tool_expanded(ui, category, name)
    local expand_icon = is_expanded and cfg.icons.expanded
      or cfg.icons.collapsed
    local name_w = (category == "lsp" or category == "dap") and cfg.col_name
      or cfg.col_tool
...
---@param ui Tool.UI
---@param category ToolCategory
---@return Tool.Table.Column[] columns
---@return Tool.Table.Row[] rows
function M.build(ui, category)
  local icon_disp_w = M.tool_icon_width()
  ...
  local row_items = core.is_ordered_category(category)
    ...
    or build_tool_rows(ui, category)
