...
local table_view = require("tool.table")
local live_update = require("tool.live_update")
local layout = require("tool.layout")
local row_model = require("tool.rows")
local help_view = require("tool.help")
local cursor = require("tool.cursor")

---@class Tool.Renderer.State
---@field ui Tool.UI?
---@field ns integer?

---@type Tool.Renderer.State
...
function M.render()
  ...
  if #rows == 0 then
    local message = _state.ui.scope == "buffer" and cfg.labels.no_current_tools
