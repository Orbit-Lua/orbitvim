...
function M.snippet_replace(snippet, fn)
  return snippet:gsub("%$%b{}", function(match)
    ...
    return n and fn({ n = n, text = name }) or match
  end) or snippet
end
...
local function setup_tab_mapping(opts)
  ...
  local tab_actions = { M.map({ "snippet_forward" }), "fallback" }
...
local function apply_blink_window_sizes(opts, sizes)
  ...
  local docs = vim.tbl_get(opts, "completion", "documentation", "window")
  if menu then
    menu.max_height = sizes.completion.height
  end
  if docs then
    docs.max_width = sizes.documentation.width
    docs.max_height = sizes.documentation.height
  end
end
